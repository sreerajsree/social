    ...
    <div class="card">
        ...
        @if ($contacts->count())
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        @foreach ($contacts as $contact)
                            <tr>
                                <td class="align-middle {{ $contact->status == '1' ? 'font-weight-bold' : '' }}">
                                    {{ $contact->created_at }}</td>
                                <td class="align-middle {{ $contact->status == '1' ? 'font-weight-bold' : '' }}">
                                    {{ $contact->name }}</td>
                                <td
                                    class="align-middle text-center {{ $contact->status == '1' ? 'font-weight-bold' : '' }}">
                                    {{ $contact->email }}</td>
                                <td
                                    class="align-middle text-center {{ $contact->status == '1' ? 'font-weight-bold' : '' }}">
                                    {{ $contact->phone != null ? $contact->phone : 'N/D' }}</td>
                                <!-- buttons -->
                                <td class="align-middle text-center">
                                    <button class="btn btn-outline-secondary mr-2"
                                        wire:click="Show({{ $contact->id }})"><i
                                            class="far {{ $contact->status == '1' ? 'fa-envelope' : 'fa-envelope-open' }} mr-1"></i>Open</button>

                                    <button class="btn btn-outline-danger"
                                        wire:click="deleteMessageConfirm({{ $contact->id }})"><i
            ...
            <div class="card-footer text-sm text-muted">
                These <strong>messages</strong> are displayed from the mailbox: <strong
                    class="text-primary">test@care.training</strong>. You can view the messages in more detail by opening
