<x-app-layout>
    ...
    <section class="bg-blue-900 py-12">
        <div class="container grid grid-cols-1 lg:grid-cols-2 gap-6 items-center">
            <figure>
                ...
                @else
                    <img id="picture" class="h-80 w-full object-cover shadow" src="{{ asset('images/courses/default.jpg') }}"
                        alt="">
                @endisset
            </figure>
            <div>
                <button type="button"
                    class="bg-opacity-25 mr-2 bg-gray-300 text-gray-300 text-sm py-2 px-4 leading-none flex items-center mb-4 focus:outline-none">
                    {{ $course->type->name }}
                </button>
                <h1 class="text-white font-extrabold text-2xl sm:text-3xl md:text-4xl">{{ $course->title }}</h1>
                <h2 class="text-white mt-3 sm:mt-5 sm:text-lg md:mt-5 md:text-xl lg:mx-0 mb-4">{{ $course->subtitle }}
                </h2>
                <p class="text-white sm:text-md md:text-lg lg:mx-0 mb-2"><span class="text-gray-400"><i
                            class="fas fa-tags text-sm mr-2"></i>Categoría:&nbsp;</span>{{ $course->category->name }}
                </p>
                <p class="text-white sm:text-md md:text-lg lg:mx-0 mb-2"><span class="text-gray-400"><i
                            class="fas fa-layer-group text-sm mr-2"></i>Nivel:&nbsp;</span>{{ $course->level->name }}
                </p>

                <div class="flex mb-4">
                    <!-- rating -->
                    <p
                        class="text-{{ $course->rating > 4 ? 'yellow' : 'white' }}-400 font-extrabold sm:text-lg sm:max-w-xl md:text-xl lg:mx-0">
                        {{ $course->rating }}
                    </p>
                    <!-- rating stars -->
                    <ul class="flex text-sm">
                        <li class="ml-2 mr-1 pt-1"><i
                                class="fas fa-star text-{{ $course->rating >= 1 ? 'yellow' : 'gray' }}-400"></i></li>
                        <li class="mr-1 pt-1"><i
                                class="fas fa-star text-{{ $course->rating >= 1 ? 'yellow' : 'gray' }}-400"></i></li>
                        <li class="mr-1 pt-1"><i
                                class="fas fa-star text-{{ $course->rating >= 1 ? 'yellow' : 'gray' }}-400"></i></li>
                        <li class="mr-1 pt-1"><i
                                class="fas fa-star text-{{ $course->rating >= 1 ? 'yellow' : 'gray' }}-400"></i></li>
                        <li class="mr-6 pt-1"><i
                                class="fas fa-star text-{{ $course->rating >= 1 ? 'yellow' : 'gray' }}-400"></i></li>
                    </ul>
                    <!-- users enrolled -->
    ...
    <div class="container grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 py-24">
        ...
        <div class="order-2 md:col-span-1 lg:col-span-2 md:order-1 lg:order-1">
            ...
            <!-- section description -->
            <section class="mb-12">
                <h2 class="font-bold text-2xl mb-12 text-gray-600">Description</h2>
            ...
            <section class="mb-12">
                <h2 class="font-bold text-2xl mb-12 text-gray-600">Requirements</h2>
                ...
                <ul class="list-disc list-inside">
                    ...
                    @empty
                        ...
                        <li class="text-gray-600 text-base">
                            This course has no assigned requirements.
                        </li>
                    @endforelse
            ...
            <section class="card mb-12">
                <div class="card-body">
                    <h2 class="font-bold text-2xl mb-2 text-gray-600">what you will learn</h2>
                    <ul class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-2">

                        @forelse ($course->goals as $goal)
                            <li class="text-gray-600 text-base"><i
                                    class="fas fa-check text-sm text-gray-500 mr-2"></i>{{ $goal->name }}</li>
                        @empty
                            <li class="text-gray-600 text-base">This course has no assigned goals</li>
                        @endforelse
            ...
            <!-- section subjects -->
            <section class="mb-12">
                <h2 class="font-bold text-2xl mb-12 text-gray-600">Course content</h2>
                ...
                @empty
                    <div class="card">
                        <div class="card-body">
                            This course has no assigned sections.
                        </div>
                    </div>
                @endforelse
            ...
            <section class="card mb-12">
                <div class="card-body">
                    ...
                    <div class="flex items-center">
                        <figure>
                            <img class="h-12 w-12 object-cover rounded-full shadow"
                                ...
                                alt="profile picture of {{ $course->editor->name }}" />
                        </figure>
                        <div class="ml-4">
                    ...
                    <form action="{{ route('admin.courses.approved', $course) }}" class="mt-4" method="POST">
                        @csrf
                        <button type="submit" class="btn-cta btn-primary btn-block mt-4 hover:shadow">pass this
                            course</button>
                    </form>

                    <a href="{{ route('admin.courses.observation', $course) }}"
                        wire:click="sendDisapprovedNotification"
                        class="btn-cta btn-block text-center mt-4 bg-transparent hover:bg-gray-500 text-gray-700 font-semibold hover:text-white border border-gray-500 hover:border-transparent rounded">Add
                        Remark</a>
